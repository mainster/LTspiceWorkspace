function Oszillator_Clapp(daten_lesen)
%------------------------------------------------------------------------
% Oszillator_Clapp
%
% Auswertung der Signale des Clapp-Oszillators
%
% Simulation: Oszillatoren/Colpitts/Clapp_Osz.sch
%------------------------------------------------------------------------
...
sim_file='..\Oszillatoren\Colpitts\Clapp_Osz.dat';
tmp_file='Oszillator_Clapp.tmp';
if (exist(tmp_file,'file') == 0) || (daten_lesen ~= 0)
    % Daten lesen und zwischenspeichern
    [p,t,d]=_read_pspice_tran(sim_file,0,'V(U1) V(U2) IC(T1) IB(T1)',4e-6);
    if isempty(t)
...
[y,z,rp1,cp1]=_oszillator_admittanz_bestimmen(t,u2,ib1,t_env,f,1e-6);
[y,z,rp2,cp2]=_oszillator_admittanz_bestimmen(t,u2,ib1,t_env,f,4e-6);
...
xlabel('t [us]');
ylabel('U1 [V]');
title('Clapp-Oszillator: Schwingkreisspannung');
...
xlabel('t [us]');
ylabel('env(U1) [V]');
title('Clapp-Oszillator: Einhuellende der Schwingkreisspannung');
...
xlabel('t [us]');
ylabel('IC1 [mA]');
title('Clapp-Oszillator: Kollektorstrom');
...
xlabel('t [us]');
ylabel('f [MHz]');
title('Clapp-Oszillator: Frequenz');
